...
public class Test03 {
	...
	static void thread() { // 1.5초후 사용자 페이지 출력할 함수 
		try {
			Thread.sleep(1500); // 1.5초후 사용자 페이지 출력
	...
	public static void main(String[] args) {
		...
		// 파일 위치경로 유지보수 용이하기 위해 상수화 하여 변수에 저장.
		final String path_START = "C:\\maeng\\resource\\";
		...
		while(true) { // 사용자 페이지
			...
			thread(); // 1.5초후 사용자 페이지 출력
			
			System.out.println("1. 상품 목록 출력");
			System.out.println("2. 구매하기");
			System.out.println("3. 프로그램 종료");
			System.out.println("입력) ");
			int action = ok(); // sc.nextInt() 정수 이외의 값을 받았을 때 처리해주는 메소드
			...
			if(action == 1) { // 상품 목록 출력
				...
				int a=0; // 모든 상품 품절일때 찾기위한 변수
				for(int i = 0; i < data.size(); i++) {
					if(data.get(i).getCnt() == 0) { // 상품이 없을때
						a++; // 하나씩 오름
					} 
				}

				if(a == data.size()) { // 위에서 모든 파일이 품절일때
					System.out.println("모든 상품이 품절입니다.\n");
					continue;
				}
				...
				while(true) { // 상품 번호 유효성 검사
					...
					int num = ok(); // sc.nextInt() 정수 이외의 값을 받았을 때 처리해주는 메소드

					for(int i = 0; i < data.size(); i++) {
						if(num == data.get(i).getPK() && 0 == data.get(i).getCnt()) { 
							System.out.println("\n선택하신 상품이 품절입니다.\n");
						} else if(num == data.get(i).getPK()) { // 존재하는 상품을 찾았을때
				...
				while (true) { // 재고 개수 유효성 검사
					...
					int cnt = ok(); // sc.nextInt() 정수 이외의 값을 받았을 때 처리해주는 메소드
				...
				int password = ok(); // sc.nextInt() 정수 이외의 값을 받았을 때 처리해주는 메소드
				...
				if(password == 971103) { // 비밀번호가 맞을시 관리자모드 진입
					...
					while(true) { // 관리자 페이지
						...
						action = ok(); // sc.nextInt() 정수 이외의 값을 받았을 때 처리해주는 메소드
						...
						if(action == 1) { // 상품 추가
							...
							while (true) {// 상품이름 유효성 검사
								...
								tmp = ok(); // sc.nextInt() 정수 이외의 값을 받았을 때 처리해주는 메소드
							...
							while (true) {
								...
								price = ok(); // sc.nextInt() 정수 이외의 값을 받았을 때 처리해주는 메소드
							...
							int action2 = ok(); // sc.nextInt() 정수 이외의 값을 받았을 때 처리해주는 메소드
							...
							if(action2 == 1) {
								...
								while(true) {
									...
									num = ok(); // sc.nextInt() 정수 이외의 값을 받았을 때 처리해주는 메소드
							...
							if(action2 == 2) { // 상품 재고 변경
								...
								while(true) {
									...
									cnt = ok(); // sc.nextInt() 정수 이외의 값을 받았을 때 처리해주는 메소드
		...
		// 프로그램을 종료할때 txt 파일에 상품목록 저장
		FileWriter fw;
		try {
			fw = new FileWriter(path_START + path_FILE01 + path_END); // 상수화 시켜놓은 경로
		} catch (IOException e) {
			e.printStackTrace();
		}
		try {
			fw = new FileWriter(path_START + path_FILE01 + path_END, true); // 파일이 있을경우 이어쓰기
			for(ProteinShake proteinShake : data) { // 상품 목록 인덱스 만큼
				fw.write(proteinShake.toString()); // 상품 목록을 txt 파일에 저장 
			}
			fw.close();
